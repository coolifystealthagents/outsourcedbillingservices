...
import { aug23Research } from './aug23-research';
import { authorizationExpiryCohort2026 } from './research-sources/authorization-expiry-cohort-2026';
import { coordinationBenefitsSequencing2026 } from './research-sources/coordination-benefits-sequencing-2026';
import { timelyFilingEvidenceChain2026 } from './research-sources/timely-filing-evidence-chain-2026';
import { telehealthPlaceServiceEvidence2026 } from './research-sources/telehealth-place-service-evidence-2026';
import { recoupmentNoticeLineage2026 } from './research-sources/recoupment-notice-lineage-2026';
import { aug31BlogBatch, aug31ResearchBatch } from './aug31-content';
import { sep1BlogBatch, sep1ResearchBatch } from './sep1-content';
import { sep2BlogBatch, sep2ResearchBatch } from './sep2-content';
import { sep3Research } from './sep3-research';
import { sep4ResearchBatch } from './sep4-content';
import { sep7ResearchBatch } from './sep7-content';
import { sep8ResearchBatch } from './sep8-content';
import { sep9ResearchBatch } from './sep9-content';
import { sep10ResearchBatch } from './sep10-content';
import { sep18ResearchBatch } from './sep18-research';
import { sep22ResearchBatch } from './sep22-research';
import { sep22bResearchBatch } from './sep22b-research';
import { sep23ResearchBatch } from './sep23-research';
import { sep24ResearchBatch } from './sep24-research';
import { sep25ResearchBatch } from './sep25-research';
...
export const researchPosts: Array<{ slug: string; title: string; excerpt: string; body: string[]; published?: string; datePublished?: string; updated?: string; featuredImage?: string; serviceCta?: { href: string; label: string; title: string; body: string } }> = [
  ...
  { slug: 'research-medical-billing-invoice-preparation-controls', title: 'Invoice Preparation Controls Research: Keeping Source Records and Approvals Separate', excerpt: 'A source-first invoice queue helps support staff prepare accurate drafts while owners retain control of terms, credits, and release.', updated: '2026-09-06', body: researchBody('Invoice preparation starts with an approved service, order, contract, or usage record. The preparation queue should show the source, billing period, customer reference, line description, quantity or unit basis, draft status, and owner for release.', 'Match each draft to the permitted source and record missing fields, unusual terms, duplicate-looking items, tax questions, credit requests, and changes to the billing schedule. Keep drafts separate from released invoices and preserve the source version used for the comparison.', 'A specialist can assemble and check a draft. The owner approves pricing terms, credits, tax treatment, customer-facing changes, and release. If the source does not support a line, stop the draft and route the exact gap.'), serviceCta: { href: '/services/invoice-preparation', label: 'Plan invoice preparation support', title: 'Turn source checks into a clear invoice-preparation role', body: 'Use the invoice preparation service guide to set approved sources, draft checks, and owner review. The authorized owner still decides terms, credits, tax treatment, customer-facing changes, and release.' } },
  { slug: 'research-medical-billing-subscription-billing-review', title: 'Subscription Billing Review Research: A Controlled Recurring-Charge Queue', excerpt: 'Recurring billing needs a dated source, a change trail, and an owner route for cancellations, pauses, upgrades, and credits.', datePublished: '2026-08-07', updated: '2026-09-09', body: researchBody('Subscription billing is repeatable only when the recurring charge has a current source. A review row should connect the customer or account reference, plan or service record, effective dates, approved change, billing cycle, draft amount, and review status.', 'Check the current subscription record against the prior cycle and approved change log. Flag starts, stops, pauses, upgrades, downgrades, duplicate charges, backdated changes, credits, and failed payment events. Do not infer a cancellation from an informal message unless the owner-approved workflow permits it.', 'The specialist may prepare the recurring charge and record a documented mismatch. The authorized owner decides plan changes, credits, refunds, exceptions, and customer-facing resolutions. Keep release rights separate from preparation.'), serviceCta: { href: '/services/subscription-billing-support', label: 'Plan subscription billing support', title: 'Set up a reviewable subscription billing queue', body: 'Use the subscription billing service guide to set the source record, change checks, and owner review. The authorized owner still decides plan changes, pauses, credits, refunds, exceptions, and customer-facing resolutions.' } },
  { slug: 'research-medical-billing-usage-billing-reconciliation', title: 'Usage Billing Reconciliation Research: Matching Activity to Billable Records', excerpt: 'A usage billing review compares activity evidence with approved rules before a charge reaches the release queue.', updated: '2026-09-11', body: researchBody('Usage billing requires a clear bridge from activity to billable quantity. The queue should identify the source event or report, period, account, rule or contract reference, units, exclusions, draft line, and reviewer.', 'Reconcile the activity report to the account and billing period, then check duplicates, missing events, cutoff errors, unit conversions, minimums, caps, and manual overrides. Preserve the report version and note any assumption that requires owner confirmation.', 'A specialist can perform the documented match and prepare an exception packet. The owner approves contract interpretation, overrides, disputed usage, credits, and release. A missing or contradictory activity source remains open until resolved.'), serviceCta: { href: '/services/usage-billing-administration', label: 'Plan a usage-billing review role', title: 'Set up a reviewable usage-billing queue', body: 'Use the usage-billing service guide to define activity sources, draft checks, and escalation records. The authorized owner still decides contract interpretation, overrides, disputed usage, credits, and release.' } },
  { slug: 'research-medical-billing-credit-memo-governance', title: 'Credit Memo Governance Research: Evidence Before Balance Changes', excerpt: 'Credit memo preparation should expose the original charge, reason, authority, and downstream reconciliation before approval.', updated: '2026-09-12', body: researchBody('A credit memo changes a customer or account balance, so preparation should begin with the original invoice or charge and the request that explains the reason. The queue should name the affected line, amount, date, source, requestor, and approval owner.', 'Check the original billing record, prior credits, payment status, dispute or service record, applicable policy, and destination ledger. Separate a request for explanation from an approved credit. Flag duplicate requests, unsupported reasons, tax implications, and credits that would create an unusual balance.', 'The specialist can assemble evidence and draft the memo. The owner approves the amount, reason, tax treatment, customer response, and posting. Never release a credit because a request appears reasonable without the required source and approval.'), serviceCta: { href: '/services/credit-memo-administration', label: 'Plan credit memo administration support', title: 'Set up a reviewable credit memo queue', body: 'Use the credit memo administration service guide to define source checks, exception records, and owner review. The authorized owner still decides the amount, reason, tax treatment, customer response, and posting.' } },
  { slug: 'research-medical-billing-collections-follow-up', title: 'Collections Follow-Up Research: Documenting Contact and Escalation', excerpt: 'A collections queue should preserve the account facts, approved contact method, response, deadline, and next decision owner.', updated: '2026-09-15', body: researchBody('Collections follow-up is a controlled status task, not permission to improvise a settlement. Each row should show the account or invoice reference, balance source, age, prior activity, approved channel, last contact, response, next date, and owner.', 'Before outreach, check the latest account record, dispute status, payment activity, contact restrictions, and any owner-approved script. Record the date, channel, factual response, promised next event, and escalation reason. Separate routine reminders from disputes, hardship requests, legal threats, privacy concerns, and settlement requests.', 'The specialist may send approved factual follow-up and record the response. The owner decides settlements, write-offs, payment plans, refunds, dispute outcomes, and changes to contact strategy. Stop when authority or source facts are unclear.'), serviceCta: { href: '/services/collections-follow-up', label: 'Plan collections follow-up support', title: 'Prepare a controlled follow-up queue', body: 'Use approved contact rules and account evidence before outreach. The authorized owner still decides settlements, write-offs, payment plans, refunds, dispute outcomes, and strategy changes.' } },
  { slug: 'research-medical-billing-customer-billing-inbox', title: 'Customer Billing Inbox Research: Routing Questions Without Losing Evidence', excerpt: 'A billing inbox becomes manageable when each message is linked to a source record, category, due date, and accountable owner.', updated: '2026-09-26', body: researchBody('A customer billing inbox should turn messages into traceable work items. Record the sender reference allowed by policy, account or invoice reference, received date, request category, source records, response deadline, status, and next owner.', 'Separate invoice questions, payment confirmations, disputes, credit requests, refund requests, access concerns, and general service questions. Link the message to the approved account record, preserve attachments in the approved system, and use owner-approved response templates. Do not make a balance change from an inbox message alone.', 'A specialist can classify, gather records, and draft a factual response. The owner approves credits, refunds, disputed balances, contract interpretation, and unusual customer-facing conclusions. Escalate threats, privacy issues, and conflicting account records.'), serviceCta: { href: '/services/customer-billing-support', label: 'Plan customer billing support', title: 'Prepare a customer billing question queue', body: 'Connect each question to the approved account record and give the owner a clear draft to review. The authorized owner still decides policy interpretation, concessions, refunds, payment commitments, and final customer responses.' } },
...
const researchAug18RepairPadding: Record<string, string> = {
  'do authorization records support the billed service date and units when the approval window is near expiration?': 'A second review should test the authorization reference against the exact claim version and service unit, not merely against the account’s current status. Keep partial approvals, amended approvals, retroactive requests, and records without a usable reference in separate categories. If the payer source uses a date convention that is not documented, preserve the displayed value and escalate the conversion question. The result should identify whether the evidence supports a date-and-unit comparison, whether an amendment changes the cohort, and what qualified owner must decide before a billing record is changed.',
  'can the available billing evidence explain which payer was primary when coordination-of-benefits records conflict?': 'The comparison should preserve claim sequence by service date and should not treat a later paid or rejected response as proof that the earlier payer order was correct. Record the source that triggered each sequence change, the date it became available, and whether the account view was refreshed after the change. Where several payer records share an abbreviated name, use stable identifiers and permitted coverage references. The final table should make clear which relationships are evidenced, which are candidate matches, and which responsibility question cannot be answered from the available record.',
  'does the enrollment evidence in a billing record support the provider, payer, location, and service date shown on the claim?': 'A useful sample includes claims that appear aligned as well as claims near enrollment starts, ends, location changes, provider-role changes, and payer file updates. Compare the historical source available at the time of billing with any later enrollment view, because a current display can conceal what was known earlier. Keep “record exists,” “relationship is dated,” and “relationship applies to this claim context” as separate findings. Escalate conflicts rather than selecting the most recent value or treating a successful transmission as proof of enrollment sufficiency.',
  'can a claim’s evidence chain establish which filing event occurred before the applicable deadline?': 'For every candidate filing event, preserve the originating system, control number, event timestamp, timezone, acknowledgment text, and claim version. A deadline calculation should show the rule source and the inputs used, including any stated extension or exception. Do not turn an internal release time into a payer receipt time, and do not infer timely acceptance from the absence of a rejection. The research output should list each missing link and state whether the owner needs a source lookup, policy interpretation, correction decision, or appeal review.',
  'do the permitted encounter and billing records contain consistent evidence for the telehealth context represented on a claim?': 'The cohort should retain ordinary encounters and ambiguous encounters so the result does not describe only obvious exceptions. Record which field identifies modality, which field supplies the location context, and which source supplied the claim value. If an encounter note is restricted, mark the evidence access-limited and avoid reconstructing the clinical context from a code or place-of-service field. A field comparison can expose a transformation or missing source; it cannot decide the correct coding treatment, document sufficiency, or payer outcome.',
  'can a recoupment or takeback notice be traced to the original claim, payment, reason, and account action?': 'Lineage is strongest when the notice is linked through stable transaction references rather than amount, patient name, or a date that happens to match. Preserve the original payment, any offset or reversal, the notice version, receipt date, response deadline, and local posting separately. If one notice covers multiple claims, retain the allocation rule or report the relationship as aggregated. A complete chain still leaves the owner to assess the notice, applicable policy, accounting treatment, patient communication, and any permitted response.',
  'can a billing evidence review identify what documentation is missing without deciding medical necessity?': 'The review should distinguish a search failure from an access boundary and from a document that was found but does not yet answer the stated request. Preserve the request version, search locations, search date, document version, and qualified-review status while minimizing protected details. If the request wording is ambiguous, quote only the necessary source language in the approved record and route an interpretation question. The finding is about reviewability and evidence availability; it must not become a clinical conclusion because the queue needs a faster status.',
  'when NPI, taxonomy, billing entity, or rendering fields differ, what can the billing source evidence actually establish?': 'The cohort should stratify mismatches by system layer and effective period, including values that match after an interface transformation and values that differ before transmission. Preserve the original field values and the reason any later value appears, with approval evidence kept in the authorized system. A payer response may identify an edit without explaining whether the source, mapping, enrollment, or taxonomy relationship caused it. The report should therefore end with a bounded owner question and never convert administrative similarity into credentialing or coding approval.',
};

const researchAug18SourceRepairAdditions: Record<string, string> = {
  'do authorization records support the billed service date and units when the approval window is near expiration?': 'A further control is to preserve the decision context at the moment the claim was prepared. Record which authorization version was available, whether the service was split across dates, and whether the unit count came from an encounter, a schedule, or a payer-defined limit. This makes a later review more precise: it can distinguish an expired source from an amended source and a unit-definition gap from an actual count difference. The research does not need to resolve every boundary case, but it should make each unresolved case reproducible and identify the owner who can interpret the applicable authorization rule.',
  'can the available billing evidence explain which payer was primary when coordination-of-benefits records conflict?': 'A further control is to anchor every comparison to the service date and the coverage evidence available for that date. Preserve changes in payer order as events, not as edits to history, and show whether the change came from eligibility, correspondence, a payer response, or an internal update. This avoids treating a later account balance as proof of responsibility. The research is complete when another reviewer can see the evidence for the sequence, the exact conflict, the missing source, and the owner question that remains; it is not complete merely because a payer name was selected in a work queue.',
  'does the enrollment evidence in a billing record support the provider, payer, location, and service date shown on the claim?': 'A further control is to compare the historical source with the claim context at the same granularity. Keep provider identity, payer relationship, location, role, and effective period as separate columns, then show which join is supported and which is not. This prevents a valid identifier from masking a missing payer relationship or a current enrollment display from masking a historical gap. The research result should be useful even when no correction is authorized: it can state the exact source needed, preserve the original claim context, and route credentialing interpretation without silently converting an administrative comparison into approval.',
  'can a claim’s evidence chain establish which filing event occurred before the applicable deadline?': 'A further control is to keep transmission evidence and adjudication evidence separate. Store the release event, clearinghouse control, acknowledgement, payer receipt, rejection, acceptance, and later correction as distinct dated observations when each is available. If a timestamp is rounded, converted, or missing a timezone, retain that limitation beside the calculation. This allows an owner to ask a precise deadline question instead of relying on a general “sent on time” label. The research can establish a supported or incomplete chain; it cannot turn an internal timestamp into payer receipt, or silence into acceptance, without a source that says so.',
  'do the permitted encounter and billing records contain consistent evidence for the telehealth context represented on a claim?': 'A further control is to compare context fields before looking at the requested outcome. Identify which source records modality, which records location, which records the service date, and which system produced the claim value. Keep audio-only, video, hybrid, in-person, and unknown states separate only when those labels are defined by the available source. This protects the analysis from code-driven assumptions. A useful result names the missing or conflicting field, the source version, and the qualified owner question. It does not select a place-of-service value or infer clinical compliance from a billing label.',
  'can a recoupment or takeback notice be traced to the original claim, payment, reason, and account action?': 'A further control is to preserve the sequence of financial events without collapsing them into one balance. Record the original payment, notice receipt, offset or repayment event, local posting, correspondence, and any superseding notice separately, with stable references and dates. If a notice aggregates claims, retain the payer allocation or mark the relationship partial. This makes amount differences and duplicate notices visible without deciding who is liable. The research conclusion should identify the strongest supported link and the missing link, then route the interpretation of contract, accounting, repayment, and patient communication to the authorized owner.',
  'can a billing evidence review identify what documentation is missing without deciding medical necessity?': 'A further control is to record the search boundary as carefully as the document result. Name the approved repositories searched, the search date, the request version, and whether access was available, while keeping unnecessary clinical detail out of the operational note. Distinguish “not found” from “not accessible” and from “found but awaiting qualified review.” These states lead to different owner questions and should not share one completion label. The research supports a precise evidence handoff: it does not determine whether a service was necessary, whether a diagnosis is correct, or whether a document satisfies a payer’s clinical standard.',
  'when NPI, taxonomy, billing entity, or rendering fields differ, what can the billing source evidence actually establish?': 'A further control is to preserve each claim version and each transformation between systems. Compare the source value, constructed claim value, transmitted value, payer response, and dated enrollment reference, then classify the mismatch by layer and effective period. A match after formatting does not prove substantive alignment, and a payer edit does not identify the responsible field without source evidence. The research should leave a compact, reproducible exception showing the original values, the conflict, the evidence still needed, and the qualified owner for credentialing, coding, or billing interpretation. No field should be changed from this comparison alone.',
};

const researchAug18FinalRepairAdditions: Record<string, string> = {
  'do authorization records support the billed service date and units when the approval window is near expiration?': 'Boundary analysis should also distinguish an authorization end date from the date on which a request was reviewed, amended, or communicated. For each service, preserve the exact unit definition and whether the source counts visits, procedures, sessions, or another measure. If the billing record uses a different unit, report the difference rather than translating it by assumption. Include cases where the authorization was renewed after service, because a later approval may explain a record without proving that the original service fell inside the earlier window. A useful cohort report therefore gives the owner a dated comparison, the source version, the unresolved rule, and the permitted next question. It does not label an item payable or nonpayable from dates alone.',
  'can the available billing evidence explain which payer was primary when coordination-of-benefits records conflict?': 'The cohort should preserve payer-order evidence at the claim and service-date level, because one account can contain several dates with different coverage relationships. Record whether an order came from a documented source, a system default, a member update, or an owner decision. Treat a secondary response as evidence about that transaction, not as retroactive proof of the primary order. When coverage records disagree, compare their effective periods and source timestamps before comparing balances. A report that names the conflict and the evidence needed to resolve it is more useful than a forced primary label. The billing support role can prepare that chronology, while the authorized owner decides sequencing, liability, communication, and any permitted claim action.',
  'does the enrollment evidence in a billing record support the provider, payer, location, and service date shown on the claim?': 'Historical enrollment review needs a join rule that is explicit about provider, payer, location, role, and effective period. A current portal snapshot may be reliable for current status while remaining silent about the service date under review. Preserve the snapshot date and the historical gap separately. If the claim carries a rendering identifier but the enrollment source is organized by billing entity, do not treat the two records as interchangeable; record the relationship that is observed and the relationship that is still unsupported. The resulting exception should show the exact fields, source versions, and date range that require credentialing or billing interpretation. It should never backfill a missing historical fact from a later approval.',
  'can a claim’s evidence chain establish which filing event occurred before the applicable deadline?': 'Deadline analysis should report the strongest dated event actually supported by the record: release, transmission, acknowledgment, payer receipt, or adjudication. These events answer different questions and may occur in different systems. Preserve the control number and timezone for each event, and identify whether an apparent duplicate is a correction, retransmission, or separate claim. If the only evidence is an internal release timestamp, call the chain incomplete rather than treating release as receipt. A cohort can still measure how often the evidence chain is complete, partially linked, or disputed. That operational finding helps an authorized owner locate the next source without making a late-filing determination or promising that a payer will accept the claim.',
  'do the permitted encounter and billing records contain consistent evidence for the telehealth context represented on a claim?': 'Telehealth evidence should be studied as a relationship among encounter context, service date, claim fields, and the source rule that defines each field. Preserve modality and place-of-service observations separately; a claim label may be transformed from another source and may not explain the underlying encounter. Include records with incomplete or restricted context so the denominator does not consist only of easy cases. If the available evidence cannot distinguish audio-only, video, hybrid, or unknown, keep the unknown category and state what source would resolve it. This approach supports a factual owner handoff about a field conflict or missing record. It does not choose a coding value, infer clinical compliance, or predict adjudication.',
  'can a recoupment or takeback notice be traced to the original claim, payment, reason, and account action?': 'A notice lineage study should treat the notice as a dated source event, not as an immediate instruction to change an account. Preserve notice number and version, receipt date, affected claim references, original payment trace, adjustment or offset, local posting, and later correspondence. If the notice covers a population, distinguish a confirmed allocation from an amount-level candidate match. Keep the original balance and later balance as separate observations so a net result does not hide a missing claim or duplicate takeback. The evidence packet can show whether the chain is complete, partial, or contradictory and can name the missing source. Contract interpretation, repayment authorization, accounting treatment, and patient communication remain owner decisions.',
  'can a billing evidence review identify what documentation is missing without deciding medical necessity?': 'Documentation-gap research is most reliable when the requested evidence is defined before the search begins. Preserve the request wording, claim or service reference, approved repositories, search date, access result, document version, and qualified-review state. “Not found” means the defined search produced no located document; “not accessible” means permission prevented a conclusion; “awaiting review” means a document was found but its sufficiency is not established. These states should not be combined for convenience. A billing support handoff may tell a qualified reviewer what was requested and where the search stopped. It may not infer medical necessity from a diagnosis, procedure, denial label, or absent file, and it may not reproduce unnecessary protected detail.',
  'when NPI, taxonomy, billing entity, or rendering fields differ, what can the billing source evidence actually establish?': 'Mismatch research should preserve the full transformation path from source record to claim transmission and payer response. Compare field names, values, effective dates, and system layers, then retain the original values beside any later approved correction. A formatting difference may be harmless, while a relationship or effective-period difference may require credentialing or billing review; the comparison alone cannot tell which. Include matched records and false-positive candidates to show whether the detection rule is overbroad. The useful output is a reproducible exception with the conflicting fields, source references, and owner question. It must not change an NPI, taxonomy, rendering relationship, or claim status, and it cannot establish credentialing sufficiency or payment outcome.',
};

const researchAug18GateRepair: Record<string, string> = {
  'do authorization records support the billed service date and units when the approval window is near expiration?': 'A boundary-date sample should preserve the authorization version, unit definition, service date, and billing release state together. Separate an amended approval from an approval that was available before service, and report date-only records as limited evidence. The result is a dated comparison for owner review, not a conclusion about coverage or payment.',
  'can the available billing evidence explain which payer was primary when coordination-of-benefits records conflict?': 'For each service date, preserve the coverage source, effective period, claim order, payer response, and event that changed the account. A later balance or secondary response cannot retroactively prove the earlier order. Report candidate sequencing and missing precedence rules separately, leaving liability and claim action with the authorized owner.',
  'does the enrollment evidence in a billing record support the provider, payer, location, and service date shown on the claim?': 'Join provider, payer, location, role, and effective period explicitly. Keep current portal status separate from historical evidence and distinguish identifier equality from a supported enrollment relationship. The useful exception preserves both claim and enrollment versions and names the credentialing or billing question that remains unresolved.',
  'can a claim’s evidence chain establish which filing event occurred before the applicable deadline?': 'Keep release, transmission, acknowledgment, payer receipt, rejection, and correction as separate dated events with their source timezone and control number. An internal release timestamp is not payer receipt. Classify the chain as complete, partial, disputed, or unresolved and let the owner interpret any exception or response route.',
  'do the permitted encounter and billing records contain consistent evidence for the telehealth context represented on a claim?': 'Compare encounter context, modality, location, service date, claim fields, and source transformation without using a code as a substitute for the encounter record. Keep restricted or missing context visible. The analysis can identify a field conflict or evidence gap, but it cannot choose coding, infer clinical compliance, or predict payer treatment.',
  'can a recoupment or takeback notice be traced to the original claim, payment, reason, and account action?': 'Preserve notice version, receipt, cited claim, original payment, offset, local posting, and later correspondence as separate events. Amount similarity is not lineage, and a complete chain does not prove the payer reason or the correct account treatment. Report complete, partial, and contradictory links for owner review.',
  'can a billing evidence review identify what documentation is missing without deciding medical necessity?': 'Define the request before searching and record approved repositories, search date, access result, document version, and review state. Not found, not accessible, and awaiting qualified review are different findings. The handoff may identify the missing component and next owner question; it may not infer medical necessity from an absent file or diagnosis label.',
  'when NPI, taxonomy, billing entity, or rendering fields differ, what can the billing source evidence actually establish?': 'Preserve values at source, claim construction, transmission, payer response, and enrollment layers. Classify formatting, mapping, effective-period, and relationship differences separately. A payer edit or field match does not settle credentialing or coding. The report should preserve the original version and route one explicit interpretation question to the qualified owner.',
};
...
const researchAug18Body = (question: string, scope: string, evidence: string, finding: string, role: string, comparison: string, comparisonTwo: string, limits: string, conclusion: string, sources: string[], ...sourceAdditions: string[]) => [
  ...
  researchAug18RepairPadding[question] ?? '',
  researchAug18SourceRepairAdditions[question] ?? '',
  researchAug18FinalRepairAdditions[question] ?? '',
  researchAug18GateRepair[question] ?? '',
  ...sourceAdditions,
...
researchPosts.push(...sep25ResearchBatch, ...sep24ResearchBatch, ...sep23ResearchBatch, ...sep22bResearchBatch, ...sep22ResearchBatch, ...sep18ResearchBatch, ...sep10ResearchBatch, ...sep9ResearchBatch);
researchPosts.push(...sep8ResearchBatch, ...sep7ResearchBatch, ...sep4ResearchBatch, ...sep3Research, ...sep2ResearchBatch, ...sep1ResearchBatch, ...aug31ResearchBatch, ...aug17ResearchDeficit, ...researchAug18Posts.slice(0, 10), ...researchAug17Posts, ...researchAug14Posts, ...researchAug13Posts, ...researchRunOrder.map((slug) => researchRunPosts.find((post) => post.slug === slug)!), ...researchBatchPosts, ...aug20Research, authorizationExpiryCohort2026, coordinationBenefitsSequencing2026, timelyFilingEvidenceChain2026, telehealthPlaceServiceEvidence2026, recoupmentNoticeLineage2026, ...aug23Research);
