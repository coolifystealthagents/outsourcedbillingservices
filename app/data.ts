...
const researchAug17Body = (title: string, question: string, evidence: string, boundary: string, implication: string) => {
  const body = researchAug14Body(title, question, evidence, boundary, implication);
  return body.map((line, index) => index === 0
    ? 'Research date: 2026-08-17. Scope: operational research for a billing owner designing a Philippines-based support workflow; this is not legal, coding, or clinical advice.'
    : line);
};
...
const researchAug17Posts: Array<{ slug: string; title: string; excerpt: string; body: string[]; published: '2026-08-17'; updated: '2026-08-17' }> = [
  { slug: 'research-medical-billing-aug17-eligibility-response-versioning', title: 'Eligibility Response Versioning Research: Preserving Which Answer Was Used', excerpt: 'A bounded study of eligibility-response versions, timestamps, and source context for billing decisions.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Eligibility Response Versioning Research', 'whether the eligibility response used for a billing decision is the correct version for the service date and benefit context', 'the version record preserves payer, member reference permitted by policy, service date, response timestamp, transaction reference, source location, and later replacement or correction events.', 'a later eligibility response may supersede an earlier one without proving that either response guarantees payment or patient responsibility.', 'retain the used response and explain any later change before an owner revises the billing decision') },
  { slug: 'research-medical-billing-aug17-claim-control-number-lineage', title: 'Claim Control Number Lineage Research: Connecting Resubmissions to Outcomes', excerpt: 'How to analyze claim control numbers across original submissions, corrections, and payer responses without collapsing distinct events.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Claim Control Number Lineage Research', 'whether original and corrected claims can be connected across control numbers, transmission events, and payer responses', 'the lineage records the original control number, replacement or corrected identifier, transmission evidence, acknowledgment, payer response, service context, and disposition of each version.', 'a linked control number proves an event relationship, not acceptance, adjudication, payment, or the correctness of a correction.', 'show each claim version and unresolved link separately so an owner can choose the next action from the actual evidence') },
  { slug: 'research-medical-billing-aug17-remittance-batch-cutoff-integrity', title: 'Remittance Batch Cutoff Integrity Research: Testing Period Boundaries', excerpt: 'A cohort method for checking whether remittance batches, deposits, posting dates, and close periods tell the same story.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Remittance Batch Cutoff Integrity Research', 'whether remittance items are assigned to the correct reporting or close period under the documented cutoff rule', 'the cohort compares remittance date, receipt date, deposit or trace reference, posting date, batch membership, reversal history, and the stated period convention.', 'a cutoff difference can reflect a legitimate system or accounting convention and does not by itself prove a posting error or misstated close.', 'make the period rule and each exception explicit before an authorized owner signs off') },
  { slug: 'research-medical-billing-aug17-document-access-denial-analysis', title: 'Billing Document Access Denial Research: Separating Missing Proof From Permission Limits', excerpt: 'How to distinguish an absent document from a document that exists but is outside the assigned access boundary.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Billing Document Access Denial Research', 'whether an unresolved billing evidence gap is caused by a missing source, an incomplete search, or a legitimate access restriction', 'the review records systems searched, filters used, access response, document reference when known, request path, stopping point, and owner responsible for granting or locating approved evidence.', 'an inaccessible document is not evidence that the underlying billing fact is absent, and access should not be bypassed to complete a queue.', 'route access-limited findings as a separate exception so the owner can resolve authorization or source ownership') },
  { slug: 'research-medical-billing-aug17-patient-responsibility-variance', title: 'Patient Responsibility Variance Research: Reconciling Source Amounts', excerpt: 'A source-led analysis of patient-responsibility differences across remittance, account, and statement records.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Patient Responsibility Variance Research', 'whether a patient-responsibility amount can be reconciled across payer evidence, account posting, and the issued statement', 'the comparison uses the payer-adjudicated amount, adjustment categories, posting events, prior corrections, statement version, and the documented rule for the balance shown.', 'a variance is a reconciliation finding, not authorization to change a balance, issue a refund, or make a patient-facing conclusion.', 'preserve component amounts and send one bounded owner question when the sources cannot be reconciled') },
  { slug: 'research-medical-billing-aug17-coding-query-turnaround-cohort', title: 'Coding Query Turnaround Cohort Research: Measuring Evidence Delay', excerpt: 'A bounded cohort approach to coding queries that separates waiting time from unresolved clinical or documentation questions.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Coding Query Turnaround Cohort Research', 'whether coding-query aging reflects source-document delay, reviewer availability, unclear routing, or another documented cause', 'the cohort records query creation, service context, requested evidence, response events, owner role, elapsed intervals, exclusions, and final disposition without treating elapsed time alone as error.', 'turnaround time does not prove coding correctness, clinical necessity, or that a delayed query could have been resolved by a support worker.', 'report the denominator and cause categories separately so the owner can improve routing without rewarding unsupported answers') },
  { slug: 'research-medical-billing-aug17-payer-portal-evidence-reproducibility', title: 'Payer Portal Evidence Reproducibility Research: Testing Repeatable Status Checks', excerpt: 'How to determine whether a payer portal status can be independently reproduced from documented search context.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Payer Portal Evidence Reproducibility Research', 'whether a second authorized reviewer can reproduce a portal status from the recorded identifiers, date, search path, and returned reference', 'the evidence captures portal or channel, search fields permitted by policy, timestamp and timezone, status wording, transaction reference, screenshots or approved record location, and known portal limitations.', 'a reproducible status check still does not prove claim acceptance, payment, or final adjudication.', 'preserve the search path and portal limitation so a later owner can challenge or confirm the result safely') },
  { slug: 'research-medical-billing-aug17-reversal-reinstatement-lineage', title: 'Billing Reversal and Reinstatement Lineage Research: Explaining State Changes', excerpt: 'A lineage model for reversals and reinstatements that keeps original postings, reasons, approvals, and replacement events connected.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Billing Reversal and Reinstatement Lineage Research', 'whether a reversal or reinstatement can be traced to its original posting, stated reason, authorization, and resulting account state', 'the lineage includes original transaction, reversal event, source notice, reason category, reviewer, approval, replacement posting, and final reconciliation result.', 'a complete event chain does not prove that the reversal reason was substantively correct or that the replacement should be released.', 'keep event reconstruction separate from approval and require the owner to resolve contradictory source evidence') },
  { slug: 'research-medical-billing-aug17-account-credit-transfer-cohort', title: 'Account Credit Transfer Cohort Research: Testing Destination Evidence', excerpt: 'A cohort analysis of account-credit transfers that distinguishes supported destinations from unresolved matching questions.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Account Credit Transfer Cohort Research', 'whether a proposed account-credit transfer has sufficient source evidence for both the originating and destination accounts', 'the cohort compares payment trace, originating balance, destination reference, service or invoice context, prior transfers, duplicate candidates, and owner authorization.', 'a plausible destination is not permission to transfer, refund, write off, or disclose account information.', 'present both account histories and the unresolved decision so an authorized owner can accept or reject the transfer') },
  { slug: 'research-medical-billing-aug17-billing-queue-exclusion-audit', title: 'Billing Queue Exclusion Audit Research: Explaining What Was Left Out', excerpt: 'Why a billing research cohort needs an auditable exclusion record before its rates or conclusions are interpreted.', published: '2026-08-17', updated: '2026-08-17', body: researchAug17Body('Billing Queue Exclusion Audit Research', 'whether excluded billing records are documented well enough to show what the reviewed cohort does and does not represent', 'the audit records the source population, inclusion rule, exclusion reason, duplicate handling, access-limited records, date window, and final denominator used for each finding.', 'a clean denominator does not make a convenience sample representative of every billing record or prove a causal explanation.', 'publish the cohort boundary and excluded count alongside the finding so owners do not mistake a selected sample for a universal rate') },
];

researchPosts.push(...researchAug17Posts, ...researchAug14Posts, ...researchAug13Posts, ...researchAug11Posts, ...researchRunOrder.map((slug) => researchRunPosts.find((post) => post.slug === slug)!), ...researchBatchPosts);
