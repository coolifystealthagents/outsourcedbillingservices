...
const dailyBlogBatch = dailyBlogSeeds.map((seed, index) => {
  ...
  return { slug: seed.slug, title: seed.title, excerpt: seed.excerpt, minutes: 10 + (index % 5), detail: {
    ...
    faqs:[{q:`Can the specialist approve a ${seed.focus} decision?`,a:'The specialist prepares evidence and identifies conflicts. The qualified owner approves coding, clinical, financial, policy, and final submission decisions.'},{q:'What happens when proof is missing?',a:'Name the missing record, record where it was checked, hold the item, and route it to the assigned owner.'},{q:'How should the queue be sampled?',a:'Use a small mixed sample the owner can review completely, then expand only after the instructions and stopping points work.'},{q:'Should sensitive details go in chat?',a:'Keep sensitive details in approved systems. Use a record reference and short issue label in the handoff unless a safer detailed channel is approved.'}],
...
researchPosts.push(...researchBatchPosts);
