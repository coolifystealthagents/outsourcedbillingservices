...
const buildDailyBlogBatch = (seeds: DailyBlogSeed[], batchTag: string) => seeds.map((seed, index) => {
  ...
  return { slug: seed.slug, title: seed.title, excerpt: seed.excerpt, minutes: 10 + (index % 5), detail: {
    keyword: seed.title, marker: `OBS-BLOG-${batchTag}-${String(index + 1).padStart(2,'0')}`, published: '2026-08-10', updated: '2026-08-10',
...
researchPosts.push(...researchBatchPosts, ...researchRunPosts);
